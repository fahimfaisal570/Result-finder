    ...
    for p_name, p_data in sorted_candidates:
        ...
        for test_reg in samples:
            ...
            if success and isinstance(res_data, dict) and len(res_data.get('Subjects', [])) > 0:
                print(f"Γ£à Empirical Match! Profile '{p_name}' owns this exam.")
                return p_name, p_data
                
    print(f"Γ¥î Empirical probe failed. No profiles contain results for this exam.")
    return None, None
...
def send_pdf_email(dept_name, pro_id, exam_name, pdf_bytes, profile_name):
    ...
    subject = f"≡ƒôè Official Exam Results: {exam_name}"
    ...
    body += "≡ƒîÉ Quick Access Dashboards:\n"
    body += "ΓÇó Result Finder: https://fec-result-finder.streamlit.app/\n"
    body += "ΓÇó Academic Analytics: https://fec-result-analytics.streamlit.app/\n\n"
    ...
    try:
        ...
        server.quit()
        print(f"Γ£à PDF Report sent successfully to {', '.join(recipients)}")
    except Exception as e:
        print(f"Γ¥î Failed to send PDF email: {e}")
